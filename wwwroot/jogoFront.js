...
class InterfaceJogo {
    ...
    async usarPoder(poder, pos = -1) {
        ...
        try {
            let resp;
            if (poder === 'embaralhar') {
                resp = await fetch(`${API_BASE}/poder/embaralhar`, {
                    method: 'POST'
                });
                ...
                resp = await fetch(`${API_BASE}/poder/dica`, {
                    method: 'POST'
                });
            } else {
    ...
    handleGameEnd() {
        ...
        if (this.modo === 'Coop') { // Modo Cooperativo
            if (this.estado.todasCartasEncontradas && !this.estado.tempoEsgotado) { // Cartas encontradas e tempo restante
                message = `Parabéns! Você revelou todas as cartas!\nPontuação Final: ${this.estado.pontuacao}`;
            } else if (this.estado.tempoEsgotado) { // Tempo esgotado
            ...
            if (this.estado.pontuacaoHumano > this.estado.pontuacaoMaquina) { // Humano ganhou
                message = `Parabéns! Você ganhou!\nSua Pontuação: ${this.estado.pontuacaoHumano}\nPontuação da Máquina: ${this.estado.pontuacaoMaquina}`;
            } else if (this.estado.pontuacaoHumano < this.estado.pontuacaoMaquina) { // IA ganhou
